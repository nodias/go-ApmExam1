...
func ApiGetUserInfo(ctx context.Context, id string) ([]byte, error) {
	...
	defer span.End()
	url := fmt.Sprintf("http://localhost:7002/userInfo/%s", id)
...
func UpdateRequestCount(ctx context.Context, name int) ([]byte, error) {
	span, ctx := apm.StartSpan(ctx, "UpdateRequestCount", "custom")
	defer span.End()
	req, _ := http.NewRequest("GET", "http://localhost:7002/hello/world", nil)
	resp, err := client.Do(req.WithContext(ctx))
	if err != nil {
		return nil, err
	}
...
func ApiGetUsers(ctx context.Context) ([]byte, error) {
	...
	defer span.End()
	req, _ := http.NewRequest("GET", "http://localhost:7002/users", nil)
	resp, err := client.Do(req.WithContext(ctx))
	if err != nil {
		return nil, err
	}
...
func ApiGetUser(ctx context.Context, uid int) ([]byte, error) {
	...
	defer span.End()
	url := fmt.Sprintf("http://localhost:7002/user/%d", uid)
	req, _ := http.NewRequest("GET", url, nil)
	resp, err := client.Do(req.WithContext(ctx))
	if err != nil {
		return nil, err
